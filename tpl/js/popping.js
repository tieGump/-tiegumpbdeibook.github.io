...
Popup.prototype={
    ...
    middle:function ()
    {
        ...
        oDialog['style']['top']=(document.documentElement.scrollTop+(document.documentElement.clientHeight)/5)+"px";
...
{g_pop=new Popup({ contentType:1,isReloadOnClose:true,width:w,height:h});
    g_pop.setContent("title","新用户注册");
    g_pop.setContent("scrollType","no");
    g_pop.setContent("contentUrl",url);
    g_pop.build();
    g_pop.show();
}
function quhui(url,w,h)
{g_pop=new Popup({ contentType:1,isReloadOnClose:true,width:w,height:h});
    g_pop.setContent("title","密码找回");
    g_pop.setContent("scrollType","no");
    g_pop.setContent("contentUrl",url);
    g_pop.build();
    g_pop.show();
